            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    if (body?.success == true && body.data != null) {
                        ...
                        Log.d("UserRepository", "Login successful, token received: ${token.take(20)}...")
                        RetrofitClient.setAuthToken(token)
                        Result.success(body.data)
                    } else {
                        Log.e("UserRepository", "Login failed: ${body?.message}")
                        Result.failure(Exception(body?.message ?: "Login failed"))
                    }
                } else {
                    Log.e("UserRepository", "Login error: ${response.code()} ${response.message()}")
